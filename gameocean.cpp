...
namespace game {
    ...
    void GameOcean::makeAlga(QPointF pos, float margin)
    {
        auto alga = m_Scene->addPixmap(QPixmap(":/img/img/alga.png")
                                       .scaled(QSize(CELL_WIDTH - margin,
                                                     CELL_HEIGHT - margin),
    ...
    {
        auto fish = m_Scene->addPixmap(QPixmap(QString(":/img/img/fish_%1.png")
                                               .arg(QString::number(qrand() % FISH_PIC_COUNT)))
                                       .scaled(QSize(CELL_WIDTH - margin,
                                                     CELL_HEIGHT - margin),
                                               Qt::IgnoreAspectRatio,
                                               Qt::SmoothTransformation)
    ...
    void GameOcean::generateItems(size_t count, Items itemType)
    {
        ...
        do {
            ...
            if (m_Scene->itemAt(itemPos.x() + CELL_WIDTH / 2,
                                itemPos.y() + CELL_HEIGHT / 2,
                                m_View->transform()) == m_Grid) {
                switch (itemType) {
                case Fish:
    ...
    {
        m_Grid = new graphics_item::Grid(CELL_COUNT_H,
                                         CELL_COUNT_V,
    ...
    GameOcean::MovePossibilities GameOcean::findPossibleStep(QGraphicsItem *item)
    {
        ...
        do {
            ...
            if (pos.x() > m_Grid->boundingRect().left() &&
                pos.y() > m_Grid->boundingRect().top() &&
                pos.x() < m_Grid->boundingRect().right() &&
                pos.y() < m_Grid->boundingRect().bottom())

                if (item->data(Name).toInt() == Fish &&
                    m_Scene->itemAt(pos.x() + CELL_WIDTH / 2,
                                    pos.y() + CELL_HEIGHT / 2,
                                    m_View->transform()) == m_Grid)
                    result.append(pos);

                if (item->data(Name).toInt() == Shark)
                    if(m_Scene->itemAt(pos.x() + CELL_WIDTH / 2,
                                       pos.y() + CELL_HEIGHT / 2,
                                       m_View->transform()) == m_Grid ||
                       m_Scene->itemAt(pos.x() + CELL_WIDTH / 2,
                                       pos.y() + CELL_HEIGHT / 2,
                                       m_View->transform())->data(Name).toInt() == Fish)
                       result.append(pos);
            ...
            if (item->data(Name).toInt() == Shark) {
                ...
                if (!p.isEmpty()) {
                    auto posWithFish = std::find_if(p.begin(), p.end(), [this](QPointF pos){
                            return m_Scene->itemAt(pos.x() + CELL_WIDTH / 2,
                                                   pos.y() + CELL_HEIGHT / 2,
                                                   m_View->transform())
                                                   ->data(Name).toInt() == Fish;});
                    if (posWithFish != p.end()) {
                        auto fish = m_Scene->itemAt(posWithFish->x() + CELL_WIDTH / 2,
                                                    posWithFish->y() + CELL_HEIGHT / 2,
                                                    m_View->transform());
